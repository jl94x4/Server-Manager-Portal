...
export const DiscoverCategoryPage: React.FC<Props> = ({ kind, id, onBack, onSelect, formatItem }) => {
    ...
    return (
        <div className="w-full flex flex-col gap-8 pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-6 p-6 rounded-2xl border border-white/10 bg-gradient-to-br from-zinc-900/90 to-black/60">
                {meta?.logoPath ? (
            ...
            {loading ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 animate-pulse px-2">
                    {[...Array(12)].map((_, i) => (
                        <div key={i} className="aspect-[2/3] rounded-xl bg-white/5 border border-white/10" />
                    ))}
                </div>
            ) : (
                <>
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 px-2">
