...
export const normalizeDiscoverySource = (value) => {
    ...
    // Phase 4 default: TMDB metadata (including unset / legacy empty).
    return 'tmdb';
};

/**
 * Effective Discover metadata source.
 * Explicit seerr/tmdb wins; unset defaults to tmdb (Phase 4 flip).
 */
...
export const applyDiscoveryQueryParams = (searchParams, path, prefs = {}, metadataLanguage = 'en') => {
    ...
    // Drop client-only locale marker so Seerr never treats it as a filter.
    ...
    if (usesLanguageAsOriginalFilter) {
        if (prefs.discoverLanguage && !params.has('language')) {
