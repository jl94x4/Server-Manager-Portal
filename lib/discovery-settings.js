...
export const normalizeRequestEngine = (value) => {
    ...
    return engine === 'portal' ? 'portal' : 'seerr';
};

/** Member request persistence engine (Phase 5 dual-run). Default remains Seerr. */
...
export const applyDiscoveryQueryParams = (searchParams, path, prefs = {}, metadataLanguage = 'en') => {
    ...
    if (usesLanguageAsOriginalFilter) {
        if (allLanguages) {
            // Home (and any allLanguages caller): keep every original language.
            // Metadata translation still comes from Accept-Language on the proxy.
