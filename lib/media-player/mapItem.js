...
export const canHttpDirectPlay = (meta = {}, { subtitleStreamId = '' } = {}) => {
    ...
    if (!BROWSER_DIRECT_VIDEO.has(vcodec)) return false;
    if (acodec && !BROWSER_DIRECT_AUDIO.has(acodec)) return false;
    return true;
};

export const pickPlayerPartId = (meta = {}) => String(firstPart(firstMedia(meta)).id || '').replace(/\D/g, '');

export const buildPlayerFileSrc = (ratingKey, { sessionId = '', offsetMs = 0 } = {}) => {
...
export const transcodeSettingsForQuality = (qualityId) => {
    if (String(qualityId || '') === ORIGINAL_PLAYER_QUALITY_ID) {
        ...
        return [
            ...
            { directPlay: '0', directStream: '1', directStreamAudio: '0', videoCodec: 'h264', audioCodec: 'aac' },
