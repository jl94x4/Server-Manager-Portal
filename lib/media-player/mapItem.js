...
export const canHttpDirectPlay = (meta = {}, {
    subtitleStreamId = '',
    allowHevc = false,
    allowAc3 = false,
} = {}) => {
    if (String(subtitleStreamId || '').replace(/\D/g, '')) return false;
    const media = firstMedia(meta);
    ...
    if (!BROWSER_DIRECT_CONTAINERS.has(container)) return false;
    ...
    const videoOk = BROWSER_DIRECT_VIDEO.has(vcodec) || (allowHevc && BROWSER_HEVC_VIDEO.has(vcodec));
    if (!videoOk) return false;
    if (acodec && !BROWSER_DIRECT_AUDIO.has(acodec) && !(allowAc3 && BROWSER_AC3_AUDIO.has(acodec))) return false;
    return true;
};
...
export const buildPlayerFileSrc = (ratingKey, {
    sessionId = '',
    offsetMs = 0,
    allowHevc = false,
    allowAc3 = false,
...
export const defaultHomeRowIds = ({ mixLibraries = false, libraries = [] } = {}) => {
    const recent = mixLibraries
        ? [...MIXED_RECENT_HOME_ROW_IDS]
        : libraries.map((row) => `recent:${String(row?.key || '').trim()}`).filter((id) => isPlayerHomeRowId(id));
...
export const mapPlayerItem = (meta = {}, { serverIdentifier = '' } = {}) => {
    ...
    return {
        ...
        viewedLeafCount: asInt(meta.viewedLeafCount),
