...
/** Poster path for cards — normalize PMS URLs and synthesize when related hubs omit thumb. */
export const pickPlayerThumb = (meta = {}) => {
    ...
    if (ratingKey) return `/library/metadata/${ratingKey}/thumb`;
    return null;
};
...
export const applyShowMetaToPlayerChild = (item = {}, mappedShow = {}) => {
    ...
    if (mappedShow.tmdbId) item.tmdbId = mappedShow.tmdbId;
    item.externalIds = {
        imdb: item.externalIds?.imdb || mappedShow.externalIds?.imdb || null,
        tmdb: mappedShow.externalIds?.tmdb || mappedShow.tmdbId || item.externalIds?.tmdb || item.tmdbId || null,
...
/** Recently Added TV should show the episode or season that was added, not the parent series. */
...
export const assembleLibraryHomeHubs = ({
    ...
    const hubs = [];
    const seen = new Set();
    const push = (identifier, title, items) => {
        const id = String(identifier || title || '').trim();
        if (!id || seen.has(id)) return;
        const rows = (Array.isArray(items) ? items : []).filter((row) => row?.ratingKey).slice(0, 24);
        if (!rows.length) return;
        seen.add(id);
        hubs.push({ title: String(title || id), identifier: id, items: rows });
    };

    if (Array.isArray(onDeckItems) && onDeckItems.length) {
        push('continueWatching', 'Continue Watching', onDeckItems);
    }
    ...
    for (const hub of asArray(sectionHubs)) {
        ...
        const items = asArray(hub?.Metadata)
            .map((meta) => mapLibraryHubItem(meta, hub, config))
            .filter((row) => row.ratingKey);
        push(identifier, hub?.title || identifier, items);
    }
    ...
    if (!hasKind(/recently\s*released|recentlyreleased|\bnewest\b/i) && newestHead && newestHead !== recentHead) {
        push('recentlyReleased', 'Recently Released', newestItems);
    }
