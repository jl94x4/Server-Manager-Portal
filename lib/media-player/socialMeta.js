...
/** Prefer landscape art for large cards; fall back to poster/thumb. */
...
export const buildMediaPlayerOgImageUrl = (baseUrl = '', ratingKey = '', { width = 1200, height = 630 } = {}) => {
    const id = String(ratingKey || '').trim();
    if (!/^\d+$/.test(id)) return '';
    const origin = String(baseUrl || '').replace(/\/+$/, '');
    const params = new URLSearchParams({
        width: String(Math.min(1600, Math.max(200, Number(width) || 1200))),
        height: String(Math.min(1600, Math.max(200, Number(height) || 630))),
    });
...
export const socialPreviewFromPlexMetadata = (payload, {
    ...
    const imageUrl = id ? buildMediaPlayerOgImageUrl(baseUrl, id) : '';
    return {
        title,
        description,
        imageUrl,
