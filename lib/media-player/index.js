...
export const createMediaPlayerRouter = ({
    ...
    router.get('/home', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const [sectionsRes, memberPromotedRes, memberHomeRes, onDeckRes, onDeckWindowWeeks] = await Promise.all([
                    sectionsPromise,
                ...
                if (!hubs) {
                    // Race server hub sources in parallel — sequential 8s timeouts made home feel stuck.
                    const [promotedRes, homeHubsRes, allHubsRes] = await Promise.all([
                    ...
                    const rawHubs = [].concat(
                        promotedRes?.MediaContainer?.Hub
                        || homeHubsRes?.MediaContainer?.Hub
                        || allHubsRes?.MediaContainer?.Hub
                        || [],
                    );
    ...
    router.get('/libraries/:sectionKey/home', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const cached = readTtlCache(libraryHomeCache, cacheKey, LIBRARY_HOME_CACHE_TTL_MS);
            if (cached) return res.json(cached);
            await withPlex(res, async ({ config, uri, token, headers }) => {
                const playback = await playbackFor(req, token);
                const hubOpts = { timeoutMs: LIBRARY_PLEX_TIMEOUT_MS };
                const [sectionRes, hubsRes, onDeckRes] = await Promise.all([
                    plexJson(fetchImpl, `${uri}/library/sections/${encodeURIComponent(sectionKey)}?X-Plex-Token=${encodeURIComponent(token)}`, headers, hubOpts).catch(() => null),
                    plexJson(fetchImpl, `${uri}/hubs/sections/${encodeURIComponent(sectionKey)}?count=12&X-Plex-Token=${encodeURIComponent(token)}`, headers, hubOpts).catch(() => null),
                    jsonForMember(
                        playback,
                ...
                if (!onDeckItems.length) {
                    onDeckItems = await memberHistoryOnDeck(req, playback, {
                        config,
                        uri,
                        sectionKey,
                        onDeckWindowWeeks,
    ...
    router.get('/item/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const metaUrl = `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeChildren=1&includeGuids=1&includeImages=1&includeMarkers=1&X-Plex-Token=${tok}`;
                ...
                const needsChildren = item.type === 'show' || item.type === 'season';
                const embedded = needsChildren ? embeddedMetaChildren(meta) : [];
                const showUrl = needsShow
                    ? `${uri}/library/metadata/${encodeURIComponent(showKey)}?includeGuids=1&includeImages=1&X-Plex-Token=${tok}`
                    : null;
                const childrenUrl = needsChildren && embedded.length === 0
                    ? `${uri}/library/metadata/${encodeURIComponent(ratingKey)}/children?excludeAllLeaves=1&X-Plex-Container-Size=500&X-Plex-Token=${tok}`
                    : null;
                const extrasUrl = `${uri}/library/metadata/${encodeURIComponent(ratingKey)}/extras?X-Plex-Token=${tok}`;
                const relatedUrl = `${uri}/hubs/metadata/${encodeURIComponent(ratingKey)}/related?count=16&X-Plex-Token=${tok}`;
                const sideOpts = { timeoutMs: ITEM_SIDE_TIMEOUT_MS };
                const onDeckPromise = needsChildren
                ...
                let children = [];
                if (needsChildren) {
                    const source = embedded.length ? embedded : metadataList(kids);
                    children = source.map((row) => mapPlayerItem(row, config));
                }
    ...
    const pipePlexBody = (plexRes, res, { asDownload = false, fileName = '' } = {}) => {
    ...
    router.get('/file/:ratingKey', requireAuth, adminOnly, async (req, res) => {
        try {
            const ratingKey = String(req.params.ratingKey || '').trim();
            if (!/^\d+$/.test(ratingKey)) return res.status(400).json({ error: 'Invalid title.' });
            await withPlex(res, async ({ uri, token }) => {
                ...
                if (!partId || (!asDownload && !canHttpDirectPlay(selected, caps))) {
                    return res.status(409).json({
                        error: asDownload
                            ? 'No downloadable media file for this title.'
                            : 'This title cannot Direct Play on this device.',
                    });
                }
                let plexRes = null;
                let lastDetail = '';
                ...
                if (!plexRes || (!plexRes.ok && plexRes.status !== 206)) {
                    return res.status(502).json({
                        error: asDownload ? 'Plex refused the download.' : 'Plex refused Direct Play.',
                        detail: lastDetail,
                    });
                }
                ...
                return pipePlexBody(plexRes, res, { asDownload, fileName });
            });
        } catch (error) {
