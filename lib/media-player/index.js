...
export const createMediaPlayerRouter = ({
    ...
    router.get('/item/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            if (!/^\d+$/.test(ratingKey)) return res.status(400).json({ error: 'Invalid title.' });
            await withPlex(res, async ({ config, uri, token, headers }) => {
                const metaUrl = `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeChildren=1&includeGuids=1&includeImages=1&includeExtras=1&includeRelated=1&includeRelatedCount=16&includeMarkers=1&X-Plex-Token=${encodeURIComponent(token)}`;
                ...
                if ((item.type === 'episode' || item.type === 'season') && showKey) {
                    const showData = await plexJson(
                        fetchImpl,
                        `${uri}/library/metadata/${encodeURIComponent(showKey)}?includeGuids=1&includeImages=1&includeRelated=1&includeRelatedCount=16&X-Plex-Token=${encodeURIComponent(token)}`,
                        headers,
                    ).catch(() => null);
                    const show = metadataList(showData)[0];
                    if (show) {
                        applyShowMetaToPlayerChild(item, mapPlayerItemDetails(show, config));
                    }
                }
                let children = [];
    ...
    const pipePlexBody = (plexRes, res, { asDownload = false, fileName = '' } = {}) => {
        res.status(plexRes.status === 206 && !asDownload ? 206 : (asDownload ? 200 : plexRes.status));
        res.setHeader('Cache-Control', 'no-store');
        ...
        if (asDownload) {
            ...
            const length = plexRes.headers.get('content-length');
    ...
    router.get('/file/:ratingKey', requireAuth, adminOnly, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                for (const pair of streamPairsFor(playback)) {
                    const headers = { ...pair.headers };
                    if (!asDownload && req.headers.range) headers.Range = String(req.headers.range);
