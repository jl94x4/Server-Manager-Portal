...
const isRetryablePlexError = (error) => {
    if (isPlexAuthError(error)) return true;
    const name = String(error?.name || '');
    const message = String(error?.message || '');
    return name === 'TimeoutError'
        || name === 'AbortError'
        || /timeout|aborted/i.test(`${name} ${message}`);
};
...
const plexJsonWithAuth = async (fetchImpl, attempts, { timeoutMs } = {}) => {
    ...
    for (const attempt of list) {
        ...
        try {
            ...
            lastError = error;
            if (!isRetryablePlexError(error)) throw error;
        }
    }
    ...
    router.get('/home', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const [sectionsRes, promotedRes] = await Promise.all([
                    plexJson(
                        fetchImpl,
                    ...
                    jsonFor(playback, `${uri}/hubs/promoted?${hubQuery}`, hubOpts).catch(() => null),
                ]);
                const sections = (sectionsRes?.MediaContainer?.Directory || [])
                    .filter((dir) => ['movie', 'show', 'artist'].includes(dir.type))
                    .map(mapPlayerSection);

                let rawHubs = [].concat(promotedRes?.MediaContainer?.Hub || []);
                if (!rawHubs.length) {
                    const homeHubsRes = await jsonFor(playback, `${uri}/hubs/home?${hubQuery}`, hubOpts).catch(() => null);
                    rawHubs = [].concat(homeHubsRes?.MediaContainer?.Hub || []);
                }
                if (!rawHubs.length) {
                    const allHubs = await jsonFor(playback, `${uri}/hubs?${hubQuery}`, hubOpts).catch(() => null);
                    rawHubs = [].concat(allHubs?.MediaContainer?.Hub || []);
                }

                let hubs = dedupeLibraryContinueWatchingHubs(mapPlayerHomeHubs(rawHubs, config));
                let continueWatching = hubs.find((hub) => isLibraryContinueWatchingHub(hub))?.items || [];
                let recentByLibrary = [];
                let playlists = [];
                if (!hubs.length) {
                    ...
                    const [onDeckRes, playlistsRes, ...recentResults] = await Promise.all([
                        plexJson(
                            fetchImpl,
                            `${uri}/library/onDeck?X-Plex-Container-Size=24&X-Plex-Token=${encodeURIComponent(onDeckToken)}`,
                            onDeckHeaders,
                            hubOpts,
                        ).catch(() => null),
                        savedSettings.showPlaylists === false
                            ? Promise.resolve(null)
                            : plexJson(fetchImpl, `${uri}/playlists?playlistType=video&X-Plex-Container-Size=24&X-Plex-Token=${encodeURIComponent(onDeckToken)}`, onDeckHeaders, hubOpts).catch(() => null),
                    ...
                    continueWatching = dedupeItems(metadataList(onDeckRes).map((meta) => mapContinueWatchingItem(meta, config)));
