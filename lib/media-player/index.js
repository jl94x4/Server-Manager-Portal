...
const PLAYER_BACKDROP_QUALITY = 85;
...
export const createMediaPlayerRouter = ({
    ...
    router.get('/hub', requireAuth, requireMember, async (req, res) => {
        try {
            const path = String(safePlexLibraryPath(req.query.path || '') || '').split('?')[0];
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                for (let start = 0; start < 2000; start += 100) {
                    const page = await plexJsonOrNull(
                        fetchImpl,
                        `${uri}${withPlexContainerParams(path, token, { start, size: 100 })}`,
                        headers,
                    );
                    if (!page) break;
                ...
                res.json({ title: title || 'Library', items });
            });
        } catch (error) {
