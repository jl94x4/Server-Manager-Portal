    ...
    router.post('/play/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const allowHevc = req.body?.canPlayHevc === true;
                const allowAc3 = req.body?.canPlayAc3 === true;
                const allowNativeHls = req.body?.canPlayNativeHls === true;
                const subtitleStreamId = options.subtitleStreamId;
                const useDirectFile = qualityId === ORIGINAL_PLAYER_QUALITY_ID
                    && canHttpDirectPlay(selected, { subtitleStreamId, allowHevc, allowAc3 });
                ...
                res.json({
                    ...
                    canDirectPlay: canHttpDirectPlay(selected, { allowHevc, allowAc3 }),
    ...
    router.get('/file/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const allowHevc = String(req.query.hevc || '') === '1';
                const allowAc3 = String(req.query.ac3 || '') === '1';
                if (!partId || !canHttpDirectPlay(selected, { allowHevc, allowAc3 })) {
                    return res.status(409).json({ error: 'This title cannot Direct Play in the browser.' });
                }
