        ...
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const useDirectFile = qualityId === ORIGINAL_PLAYER_QUALITY_ID
                    && canHttpDirectPlay(playable, { subtitleStreamId: options.subtitleStreamId });
                res.json({
                    sessionId,
                    item: playableItem,
                    src: useDirectFile
                        ? buildPlayerFileSrc(playableItem.ratingKey, { sessionId, offsetMs: offset })
                ...
                for (const playToken of tokens) {
                    ...
                    for (const attempt of attempts) {
                        const params = new URLSearchParams({
                            ...
                            ...attempt,
                            directPlay: subtitleStreamID ? '0' : (attempt.directPlay || '0'),
    ...
    router.get('/file/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                if (!partId || !canHttpDirectPlay(meta)) {
