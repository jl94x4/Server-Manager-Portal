    ...
    router.get('/home', requireAuth, requireMember, async (req, res) => {
        try {
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const [sectionsRes, promotedRes, onDeckRes] = await Promise.all([
                    plexJson(fetchImpl, `${uri}/library/sections?X-Plex-Token=${encodeURIComponent(token)}`, headers),
                    jsonFor(playback, `${uri}/hubs/promoted?count=16&includeMeta=1`).catch(() => null),
                    plexJson(
                        fetchImpl,
                ...
                if (!rawHubs.length) {
                    const homeHubsRes = await jsonFor(playback, `${uri}/hubs/home?count=16&includeMeta=1`).catch(() => null);
                    rawHubs = [].concat(homeHubsRes?.MediaContainer?.Hub || []);
                }
                if (!rawHubs.length) {
                    const allHubs = await jsonFor(playback, `${uri}/hubs?count=16&includeMeta=1`).catch(() => null);
                    rawHubs = [].concat(allHubs?.MediaContainer?.Hub || []);
                }

                const continueWatching = dedupeItems(metadataList(onDeckRes).map((meta) => mapContinueWatchingItem(meta, config)));
                let hubs = dedupeLibraryContinueWatchingHubs(mapPlayerHomeHubs(rawHubs, config));
                if (continueWatching.length && !hubs.some((hub) => isLibraryContinueWatchingHub(hub))) {
                    hubs = [{
