...
export const createMediaPlayerRouter = ({
    ...
    const playbackFor = async (req, serverToken) => {
        ...
        const serverHeaders = playerHeaders(serverToken, STREAM_IDENTITY);
        return {
            token,
            streamToken: serverToken,
            identity: userIdentity,
            streamIdentity: STREAM_IDENTITY,
            headers: streamHeaders(token, userIdentity),
            streamAuthHeaders: streamHeaders(serverToken, STREAM_IDENTITY),
            metaHeaders: playerHeaders(token, userIdentity),
            serverHeaders,
            jsonAttempts: (urlForToken) => [
                { token, url: urlForToken(token), headers: playerHeaders(token, userIdentity) },
        ...
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const attemptHeaders = {
                    ...streamHeaders(playToken, playback.streamIdentity),
    ...
    router.get('/file/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const headers = { ...streamHeaders(playback.streamToken, playback.streamIdentity) };
    ...
    router.post('/timeline', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                if (!plexRes.ok) {
                    if (plexRes.status === 401 || plexRes.status === 403) return res.status(204).end();
                    const detail = String(await plexRes.text().catch(() => '')).slice(0, 200);
    ...
    router.get('/proxy', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const headers = { ...streamHeaders(config.plexToken, STREAM_IDENTITY) };
            if (req.headers.range) headers.Range = String(req.headers.range);
