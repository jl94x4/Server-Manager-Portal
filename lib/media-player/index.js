...
export const createMediaPlayerRouter = ({
    ...
    const playbackFor = async (req, serverToken) => {
        ...
        return {
            ...
            streamAuthHeaders: streamHeaders(serverToken, identity),
    ...
    const fetchNeighborEpisode = async (uri, token, headers, ratingKey, direction) => {
        ...
        const kids = metadataList(await plexJson(
            fetchImpl,
            `${uri}/library/metadata/${encodeURIComponent(neighborSeason.ratingKey)}/children?excludeAllLeaves=1&X-Plex-Container-Size=500&X-Plex-Token=${encodeURIComponent(token)}`,
            headers,
        ).catch(() => null));
    ...
    const fetchNextEpisode = (uri, token, headers, ratingKey) => (
        fetchNeighborEpisode(uri, token, headers, ratingKey, 1)
    );
    ...
    router.get('/home', requireAuth, requireMember, async (req, res) => {
        try {
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const [onDeckRes, playlistsRes, ...recentResults] = await Promise.all([
                    ...
                    savedSettings.showPlaylists === false
                        ...
                        : plexJson(fetchImpl, `${uri}/playlists?playlistType=video&X-Plex-Container-Size=24&X-Plex-Token=${encodeURIComponent(onDeckToken)}`, onDeckHeaders).catch(() => null),
                ...
                const playlists = savedSettings.showPlaylists === false
                    ? []
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                for (const hub of data?.MediaContainer?.Hub || []) {
                    for (const meta of hub.Metadata || []) {
                        ...
                        results.push(meta.type === 'playlist'
    ...
    router.get('/libraries/:sectionKey/home', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                let addedContinue = onDeckItems.length > 0;
    ...
    router.get('/collection/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                const playback = await playbackFor(req, token);
                const plexToken = playback.token || token;
                const plexHeaders = playback.metaHeaders || headers;
                const metaRes = await plexJson(
                    fetchImpl,
                    `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeChildren=1&includeGuids=1&X-Plex-Token=${encodeURIComponent(plexToken)}`,
                    plexHeaders,
                );
                ...
                const takeChildren = (payload) => {
                    const rows = [];
                    for (const row of collectionChildItems(payload, ratingKey)) {
                ...
                if (!childMetas.length) {
                    for (const path of collectionChildPaths(meta, ratingKey)) {
        ...
        try {
            ...
            await withPlex(res, async ({ config, uri, token }) => {
                ...
                const [previous, next] = await Promise.all([
                    fetchNeighborEpisode(uri, playback.token, playback.metaHeaders, ratingKey, -1),
                    fetchNeighborEpisode(uri, playback.token, playback.metaHeaders, ratingKey, 1),
                ]);
                res.json({
    ...
    router.get('/item/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                let onDeck = null;
                if (item.type === 'show' || item.type === 'season') {
                    const playable = await resolvePlayableMeta(uri, token, headers, ratingKey, meta);
    ...
    router.post('/play/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const data = await plexJson(
                    fetchImpl,
                    `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeMarkers=1&X-Plex-Token=${encodeURIComponent(playback.token)}`,
                    playback.metaHeaders,
                );
                const meta = metadataList(data)[0];
                ...
                if (String(playable.ratingKey) !== String(ratingKey)) {
                    const playableData = await plexJson(
                        fetchImpl,
                        `${uri}/library/metadata/${encodeURIComponent(playable.ratingKey)}?includeMarkers=1&X-Plex-Token=${encodeURIComponent(playback.token)}`,
                        playback.metaHeaders,
                    ).catch(() => null);
                    playableMeta = metadataList(playableData)[0] || playable;
                }
                const mediaIndex = pickMediaIndex(req.body?.mediaIndex, playableMeta);
                ...
                const options = mapPlayerPlaybackOptions(selected, {
                    audioLanguage: req.body?.audioLanguage ?? savedSettings.audioLanguage,
                    subtitleMode: req.body?.subtitleMode ?? savedSettings.subtitleMode,
                });
                ...
                const allowNativeHls = req.body?.canPlayNativeHls === true || caps.client !== 'web';
                ...
                res.json({
                    ...
                    canDirectPlay: canHttpDirectPlay(selected, { ...req.body, subtitleStreamId: '' }),
    ...
    router.get('/hls/:ratingKey/master.m3u8', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                const playback = await playbackFor(req, token);
                let lastDetail = '';
                const playToken = playback.streamToken || token;
                ...
                for (const attempt of attempts) {
                    ...
                    const params = new URLSearchParams({
                        ...
                        ...plexAttempt,
                        directPlay: subtitleStreamID ? '0' : (plexAttempt.directPlay || '0'),
                    ...
                    addPlexIdentityParams(params, attemptHeaders, playToken, { transcodeProfile: !copy });
    ...
    router.get('/file/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const headers = { ...streamHeaders(playback.streamToken, playback.identity) };
                if (req.headers.range) headers.Range = String(req.headers.range);
                const plexRes = await fetchImpl(
                    `${uri}/library/parts/${encodeURIComponent(partId)}/file?X-Plex-Token=${encodeURIComponent(playback.streamToken)}`,
                    { headers },
                );
                if (!plexRes.ok && plexRes.status !== 206) {
                    const detail = String(await plexRes.text().catch(() => '')).slice(0, 200);
                    return res.status(502).json({ error: 'Plex refused Direct Play.', detail });
                }
                ...
                res.send(Buffer.from(await plexRes.arrayBuffer()));
            });
        } catch (error) {
    ...
    router.post('/stop', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                addPlexIdentityParams(params, playback.streamAuthHeaders, playback.streamToken);
                await fetchImpl(`${uri}/video/:/transcode/universal/stop?${params}`, {
                    headers: {
                        ...playback.streamAuthHeaders,
        ...
        try {
            ...
            await withPlex(res, async ({ config, uri, token }) => {
                ...
                const [metaRes, itemsRes] = await Promise.all([
                    plexJson(
                        fetchImpl,
                        `${uri}/playlists/${encodeURIComponent(ratingKey)}?X-Plex-Token=${encodeURIComponent(playback.token)}`,
                        playback.metaHeaders,
                    ),
                    plexJson(
                        fetchImpl,
                        `${uri}/playlists/${encodeURIComponent(ratingKey)}/items?X-Plex-Container-Start=0&X-Plex-Container-Size=200&X-Plex-Token=${encodeURIComponent(playback.token)}`,
                        playback.metaHeaders,
                    ).catch(() => null),
                ]);
                const meta = metadataList(metaRes)[0];
    ...
    router.get('/proxy', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const identity = identityKey(req);
            const headers = { ...streamHeaders(config.plexToken, identity) };
            if (req.headers.range) headers.Range = String(req.headers.range);
            const plexRes = await fetchImpl(target, { headers });
