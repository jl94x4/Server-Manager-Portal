...
export const createMediaPlayerRouter = ({
    ...
    const handlePlay = async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                const playback = await playbackFor(req, token);
                const metaUrl = (authToken) => (
                    `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeMarkers=1&X-Plex-Token=${encodeURIComponent(authToken)}`
                );
                let data;
                try {
                    data = await plexJson(fetchImpl, metaUrl(playback.token), playback.metaHeaders, { timeoutMs: 15000 });
                } catch (error) {
                    if (playback.token === token) throw error;
                    data = await plexJson(fetchImpl, metaUrl(token), headers, { timeoutMs: 15000 });
                }
                const meta = metadataList(data)[0];
