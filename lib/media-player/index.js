            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                if (asDownload) {
                    const rawFile = String(
                        selected?.Media?.[0]?.Part?.[0]?.file
                        || meta?.title
                        || `plex-${ratingKey}`,
                    );
                    const fileName = rawFile.split(/[/\\]/).pop() || `plex-${ratingKey}`;
    ...
    router.delete('/progress/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const plexRes = await fetchImpl(`${uri}/:/progress?${params.toString()}`, {
                    method: 'DELETE',
                    headers: playback.metaHeaders,
                });
                if (!plexRes.ok && plexRes.status !== 404) {
                    const detail = String(await plexRes.text().catch(() => '')).slice(0, 200);
                    return res.status(502).json({ error: 'Plex refused to clear progress.', detail });
                }
                clearHomeCaches(req);
                res.status(204).end();
            });
