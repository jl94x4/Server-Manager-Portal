        ...
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const useDirectFile = qualityId === ORIGINAL_PLAYER_QUALITY_ID
                    && canHttpDirectPlay(playable, { subtitleStreamId, allowHevc, allowAc3 });
                const copyOriginal = qualityId !== ORIGINAL_PLAYER_QUALITY_ID
                    || !isHevcVideo(playable)
                    || (allowHevc && allowNativeHls);
                res.json({
                    sessionId,
                    item: playableItem,
                    src: useDirectFile
                        ? buildPlayerFileSrc(playableItem.ratingKey, {
                            sessionId,
                            offsetMs: offset,
                            allowHevc,
                            allowAc3,
                        })
                        : buildPlayerHlsSrc(playableItem.ratingKey, {
                            sessionId,
                            offsetMs: offset,
                            qualityId,
                    ...
                    ...options,
                    qualityId,
                    canDirectPlay: canHttpDirectPlay(playable, { allowHevc, allowAc3 }),
    ...
    router.get('/file/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token }) => {
                ...
                const meta = metadataList(data)[0];
                const partId = pickPlayerPartId(meta);
                const allowHevc = String(req.query.hevc || '') === '1';
                const allowAc3 = String(req.query.ac3 || '') === '1';
                if (!partId || !canHttpDirectPlay(meta, { allowHevc, allowAc3 })) {
