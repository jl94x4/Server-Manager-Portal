...
export const createMediaPlayerRouter = ({
    ...
    router.get('/home', requireAuth, requireMember, async (req, res) => {
        try {
            await withPlex(res, async ({ config, uri, token, headers }) => {
                ...
                const continueWatching = metadataList(onDeckRes).map((meta) => mapPlayerItem(meta, config));
                const recentMovies = [];
                const recentShows = [];
                const recentMusic = [];
    ...
    router.get('/item/:ratingKey', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ config, uri, token, headers }) => {
                const data = await plexJson(
                    fetchImpl,
                    `${uri}/library/metadata/${encodeURIComponent(ratingKey)}?includeChildren=1&X-Plex-Token=${encodeURIComponent(token)}`,
                    headers,
                );
    ...
    router.get('/hls/:sessionId/master.m3u8', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            await withPlex(res, async ({ uri, token, headers }) => {
                ...
                if (!plexRes.ok) {
                    return res.status(502).json({ error: 'Plex refused to start transcode.', detail: body.slice(0, 300) });
                }
    ...
    router.get('/proxy', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const plexRes = await fetchImpl(target, { headers: playerHeaders(config.plexToken) });
