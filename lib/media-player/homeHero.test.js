...
test('buildMediaPlayerHomeHero matches TMDB trending via GUID and caches', async () => {
    ...
    assert.ok(first.items[0].backdropUrl.includes('/w1280/fight-hi.jpg'));
    ...
    assert.ok(cached.items[0].backdropUrl.includes('/w1280/fight-hi.jpg'));
    clearMediaPlayerHomeHeroCache();
});
...
test('buildMediaPlayerHomeHero falls back to Plex title search', async () => {
    ...
    assert.ok(payload.items[0].backdropUrl.includes('/w1280/inception-hi.jpg'));
    clearMediaPlayerHomeHeroCache();
});
