...
test('HTTP Direct Play is only offered for browser-safe MP4 files', () => {
    ...
    // TrueHD is not in stock ExoPlayer. Direct Play would be a black screen.
    assert.equal(canHttpDirectPlay(mkv, { client: 'android', subtitleStreamId: '40' }), false);
    assert.equal(canHttpDirectPlay({
        Media: [{
            container: 'mkv',
            videoCodec: 'h264',
...
test('Direct Play on Android uses the requested audio stream, not only the selected TrueHD track', () => {
    const mkv = {
        Media: [{
            container: 'mkv',
            videoCodec: 'hevc',
    ...
    assert.equal(canHttpDirectPlay(mkv, { client: 'android' }), false);
    assert.equal(canHttpDirectPlay(mkv, { client: 'android', audioStreamID: '21' }), true);
    assert.equal(playbackCapsFromRequest({
        query: {},
        headers: { 'user-agent': 'SMP-MediaPlayer/1.0 (Android TV; ExoPlayer)' },
    }).client, 'android');
