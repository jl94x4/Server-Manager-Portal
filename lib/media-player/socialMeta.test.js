...
test('pickMediaPlayerSocialImagePath prefers art over stills', () => {
    assert.equal(pickMediaPlayerSocialImagePath({
        art: '/library/metadata/1/art/2',
        thumb: '/library/metadata/1/thumb/3',
    }), '/library/metadata/1/art/2');
    assert.equal(pickMediaPlayerSocialImagePath({
        thumb: 'https://evil.example/x',
    }), '');
});

test('socialPreviewFromPlexMetadata builds OG fields and a public image URL', () => {
    ...
    assert.equal(
        preview.imageUrl,
        'https://portal.example/api/public/media-player/og-image/813130?width=1200&height=630',
    );
});
...
test('buildSocialMetaTagBlock emits Open Graph and Twitter tags', () => {
    const html = buildSocialMetaTagBlock({
        ...
        pageUrl: 'https://portal.example/media-player/item/813130',
        imageUrl: 'https://portal.example/api/public/media-player/og-image/813130?width=1200&height=630',
        siteName: 'SubZero',
        type: 'video.movie',
    });
    ...
    assert.equal(buildMediaPlayerOgImageUrl('https://x/', '12'), 'https://x/api/public/media-player/og-image/12?width=1200&height=630');
