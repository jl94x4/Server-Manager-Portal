...
export const isDeclinedSeerrRequest = (reqItem) => Number(reqItem?.status) === 3;

export const isApprovedSeerrRequest = (reqItem) => Number(reqItem?.status) === 2;
...
export const resolveSeerrListQuery = (portalFilter, take = 20) => {
    if (portalFilter === 'declined') {
        return {
            apiFilter: 'all',
            clientFilter: 'declined',
            take: Math.min(100, Math.max(take * 4, 50)),
        };
    }
...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, resolveRequestAppFetchUrl }) => {
    ...
    const listRequests = async (config, { filter = 'pending', take = 20, skip = 0, sort = 'added' } = {}) => {
        ...
        const { apiFilter, clientFilter, take: fetchTake } = resolveSeerrListQuery(filter, take);
        const fetchPage = async (filterValue, sortValue, pageTake, pageSkip) => {
        ...
        let payload = await fetchPage(apiFilter, sort, fetchTake, skip);
        let results = Array.isArray(payload?.results) ? payload.results : [];

        if (filter === 'pending' && results.length === 0) {
            const fallbackPayload = await fetchPage('all', 'added', fetchTake, skip).catch(() => null);
        ...
        if (clientFilter) {
            results = applyPortalRequestStatusFilter(results, clientFilter).slice(0, take);
        }
