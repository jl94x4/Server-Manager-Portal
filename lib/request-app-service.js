...
import { enrichTvDetailsWithSonarrLibraryStatus } from './sonarr-library-status.js';
...
export const hasSeerrPermission = (permissions, ...bits) => {
    ...
    if ((value & SeerrPermission.MANAGE_REQUESTS) === SeerrPermission.MANAGE_REQUESTS) return true;
...
export const memberRequestDisplayStatus = (item = {}) => {
    ...
    if (status === 2 && mediaStatus === 3) {
        return hasActiveSeerrDownloads(media, { is4k }) ? 'Processing' : 'Requested';
    }
...
export const createRequestAppService = ({ fetchWithTimeout, resolveIntegrationUrlForFetch, resolveRequestAppFetchUrl }) => {
    ...
    const getMemberRequestCounts = async (config, sessionUser) => {
        ...
        const all = await fetchAllMemberRequests(config, userId);
    ...
    const getMemberWatchlist = async (config) => {
    ...
    const requestMemberWatchlist = async (config, sessionUser, { all = false, items = [], is4k = false } = {}) => {
        ...
        let targets = [];
        if (all) {
            const watchlist = await getMemberWatchlist(config);
    ...
    const getMemberRequestOptions = async (config, sessionUser, { mediaType, mediaId }) => {
        ...
        const [detailsRaw, users, servers] = await Promise.all([
        ...
        if (type === 'tv' && detailsRaw && typeof detailsRaw === 'object') {
            try {
                details = await enrichTvDetailsWithSonarrLibraryStatus(config, detailsRaw, {
        ...
        const canRequestBase = hasSeerrPermission(permissions, SeerrPermission.REQUEST) && !isBlacklisted;
        const canRequest4k = type === 'movie'
            ? hasSeerrPermission(
                permissions,
        ...
        if (canRequest && standardQuota?.limit > 0 && standardQuota.remaining === 0) {
            canRequest = false;
        ...
        return {
            ...
            hasHdServer,
            permissions: {
                request: hasSeerrPermission(permissions, SeerrPermission.REQUEST),
