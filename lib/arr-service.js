...
export const fetchArrRootFolders = async (instance, { resolveUrl = (url) => url, fetchImpl = fetch } = {}) => {
    const payload = await fetchArrInstanceJson(instance, '/api/v3/rootfolder', { resolveUrl, fetchImpl });
    if (!Array.isArray(payload)) return [];
    return payload.map((entry) => ({
        id: Number(entry.id),
        path: String(entry.path || ''),
        freeSpace: entry.freeSpace ?? null,
    })).filter((entry) => entry.path && Number.isFinite(entry.id));
};

export const fetchArrTags = async (instance, { resolveUrl = (url) => url, fetchImpl = fetch } = {}) => {
    const payload = await fetchArrInstanceJson(instance, '/api/v3/tag', { resolveUrl, fetchImpl });
    if (!Array.isArray(payload)) return [];
    return payload.map((entry) => ({
        id: Number(entry.id),
...
export const fetchArrLanguageProfiles = async (instance, { resolveUrl = (url) => url, fetchImpl = fetch } = {}) => {
    const payload = await fetchArrInstanceJson(instance, '/api/v3/languageprofile', { resolveUrl, fetchImpl });
    if (!Array.isArray(payload)) return [];
    return payload.map((entry) => ({
        id: Number(entry.id),
...
/**
 * Add a series to Sonarr (or return existing) and optionally search missing.
 * @returns {{ ok: boolean, created: boolean, entity?: object, reason?: string }}
 */
export const addSonarrSeries = async (instance, {
    ...
    if (!lookup) return { ok: false, created: false, reason: 'Sonarr could not look up this series on TMDB' };

    if (lookup.id) {
        if (search) {
            await triggerArrEntitySearch(instance, lookup, 'sonarr', fetchOpts);
        }
        return { ok: true, created: false, entity: lookup };
    }

    const selectedSeasons = seasons === 'all'
        ? null
        : new Set(
    ...
    const nextSeasons = Array.isArray(lookup.seasons)
        ? lookup.seasons.map((season) => {
            const seasonNumber = Number(season?.seasonNumber);
            const isSpecial = seasonNumber === 0;
            const shouldMonitor = selectedSeasons == null
                ? (!isSpecial && monitored !== false)
                : selectedSeasons.has(seasonNumber);
            return { ...season, monitored: !!shouldMonitor };
        })
        : [];
    ...
    const body = {
        ...
        addOptions: {
            ...
            monitor: selectedSeasons == null ? 'all' : 'selected',
        },
    };
