...
export const fetchSonarrAllEpisodeFiles = async (instance, { resolveUrl = (url) => url, fetchImpl = fetch } = {}) => {
    const payload = await fetchArrInstanceJson(instance, '/api/v3/episodefile', { resolveUrl, fetchImpl });
    return Array.isArray(payload) ? payload : [];
};
