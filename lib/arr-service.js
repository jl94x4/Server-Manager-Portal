...
/** Match a series already in Sonarr by TMDB and/or TVDB id. */
const findSonarrSeriesInLibrary = async (instance, { tmdbId = null, tvdbId = null } = {}, fetchOpts = {}) => {
    ...
    if (Number.isFinite(tmdb) && tmdb > 0) {
        const byTmdb = list.find((series) => Number(series?.tmdbId) === tmdb);
        if (byTmdb) return byTmdb;
    }
    if (Number.isFinite(tvdb) && tvdb > 0) {
        const byTvdb = list.find((series) => Number(series?.tvdbId) === tvdb);
...
export const addSonarrSeries = async (instance, {
    ...
    const syncExistingSeries = async (seed) => {
        const existing = (seed?.id
            ? (await fetchSonarrSeriesById(instance, seed.id, fetchOpts) || seed)
            : seed);
        if (!existing?.id) return { ok: false, created: false, reason: 'Sonarr series id missing' };
        ...
        const updated = await fetchArrInstance(instance, `/api/v3/series/${existing.id}`, {
            ...
            method: 'PUT',
            body: putBody,
            // Don't let a stuck Sonarr PUT turn into an nginx blank 502 for "already there" retries.
            timeoutMs: 25000,
        });

        const entity = updated?.ok ? (updated.data || existing) : existing;
        if (search) {
            await triggerArrEntitySearch(instance, entity, 'sonarr', { ...fetchOpts, timeoutMs: 15000 }).catch(() => null);
        }
        // Already in Sonarr — treat as success even when season update fails/times out.
        return {
            ok: true,
            created: false,
            entity,
            ...(updated?.ok ? {} : { reason: 'Already in Sonarr (season sync skipped)' }),
        };
    };

    // Catalog-first (Radarr parity) — lookup often omits id even when the series is present.
    const libraryHit = await findSonarrSeriesInLibrary(instance, {
        tmdbId: wantedTmdb,
        tvdbId: wantedTvdb,
    }, fetchOpts);
    if (libraryHit?.id) {
        return syncExistingSeries(libraryHit);
    }
    ...
    if (lookup.id) {
        return syncExistingSeries(lookup);
    }

    const tvdbId = Number(lookup.tvdbId) || Number(wantedTvdb);
    ...
    if (!result?.ok) {
        if (arrErrorLooksLike(result, /already been added|series.*exist|has already been added/i)) {
            const raced = await findSonarrSeriesInLibrary(instance, {
                tmdbId: wantedTmdb || lookup.tmdbId,
                tvdbId: tvdbId,
            }, fetchOpts);
            if (raced?.id) return syncExistingSeries(raced);
            // Still treat as success — Sonarr confirms it exists even if catalog race lags.
            return { ok: true, created: false, entity: raced || lookup, reason: 'Already in Sonarr' };
        }
        if (arrErrorLooksLike(result, /malformed|corrupt|disk image/i)) {
            return {
                ok: false,
