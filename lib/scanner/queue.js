...
/**
 * @typedef {{ folder: string, priority: number, time: string, source?: string }} ScanItem
 */
...
export const upsertScans = async (items = []) => {
    ...
    for (const item of items) {
        ...
        if (next.priority > existing.priority) {
            ...
            if (nextMs < existingMs) byFolder.set(folder, { ...existing, time: next.time, source: next.source });
        }
    }
