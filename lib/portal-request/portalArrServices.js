...
export const inferArrInstanceIs4k = (instance) => {
    const nameIs4k = nameLooksLike4kServer(instance?.name);
    // Explicit true always wins. Explicit false must not hide clearly named 4K instances
    // (ArrInstancesPanel defaults is4k to false for every new row).
    if (instance?.is4k === true) return true;
    if (nameIs4k) return true;
...
export const inferServerIs4k = (server = {}) => {
    ...
    if (server?.is4k === false || server?.is4k === 0 || server?.is4k === 'false') return false;
    return false;
};
