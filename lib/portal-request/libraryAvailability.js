...
export const createLibraryAvailability = (config = {}, options = {}) => {
    ...
    const snapshotLibraryStatuses = async () => {
        ...
        /** tmdbId → tvdbId for Sonarr rows that only matched via TVDB */
        const tvdbHintByTmdb = new Map();

        // Sonarr often stores tvdbId but not tmdbId — resolve during the 12h scan only
        // (never on the request path). Bounded concurrency keeps Node responsive.
        const missingTvdbIds = [];
        for (const [tvdb, entry] of seriesIndexes.byTvdb.entries()) {
            ...
            if (Number.isFinite(existingTmdb) && existingTmdb > 0) {
                ...
                tvdbHintByTmdb.set(existingTmdb, Number(tvdb));
                continue;
            }
            const tvdbNum = Number(tvdb);
            if (Number.isFinite(tvdbNum) && tvdbNum > 0) missingTvdbIds.push(tvdbNum);
        }

        const resolveConcurrency = 3;
        for (let i = 0; i < missingTvdbIds.length; i += resolveConcurrency) {
            const slice = missingTvdbIds.slice(i, i + resolveConcurrency);
            await Promise.all(slice.map(async (tvdbId) => {
                const tmdbId = await resolveTmdbIdFromTvdb(config, tvdbId, {
                    fetchImpl: fetchOpts.fetchImpl || fetch,
                });
                if (!tmdbId) return;
                seriesIdSet.add(tmdbId);
                tvdbHintByTmdb.set(tmdbId, tvdbId);
            }));
        ...
        for (let i = 0; i < seriesIds.length; i += chunkSize) {
            ...
            await Promise.all(slice.map(async (tmdbId) => {
                const status = await getTvListStatus(tmdbId, {
                    ...
                    tvdbId: tvdbHintByTmdb.get(tmdbId) || null,
                });
                if (!status?.mediaInfo) return;
                byKey[`tv:${tmdbId}`] = {
                    mediaInfo: status.mediaInfo,
