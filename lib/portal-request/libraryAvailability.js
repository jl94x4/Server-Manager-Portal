...
export const createLibraryAvailability = (config = {}, options = {}) => {
    ...
    const getTvListStatus = async (tmdbId, { title = '', year = null, tvdbId: tvdbIdHint = null } = {}) => {
        const id = Number(tmdbId);
        if (!Number.isFinite(id) || id <= 0) {
            return {
                mediaType: 'tv',
    ...
    const snapshotLibraryStatuses = async () => {
        ...
        for (let i = 0; i < seriesIds.length; i += chunkSize) {
            ...
            // Catalog stats are Sonarr's own monitored % — fast enough for the 2‑minute cache rebuild.
