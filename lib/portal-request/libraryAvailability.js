...
const movieCatalogCache = new Map(); // key → { at, byTmdb: Map }
const seriesCatalogCache = new Map(); // key → { at, byTmdb: Map }
...
const seriesStatsStatus = (series = {}) => {
    ...
    // In Sonarr but no files yet — tracked / processing.
...
const loadMovieCatalogIndex = async (config, fetchOpts) => {
    ...
    await Promise.all(instances.map(async (instance) => {
        ...
        list = await fetchArrInstanceCatalogItems(instance, fetchOpts).catch(() => []);
        const index = new Map();
        for (const movie of list) {
            const tmdb = Number(movie?.tmdbId);
            if (!Number.isFinite(tmdb) || tmdb <= 0) continue;
            const entry = { movie, instance };
            index.set(tmdb, entry);
            if (!byTmdb.has(tmdb)) byTmdb.set(tmdb, entry);
        }
        movieCatalogCache.set(key, { at: now(), list, byTmdb: index });
    }));
...
const loadSeriesCatalogIndex = async (config, fetchOpts) => {
    ...
    await Promise.all(instances.map(async (instance) => {
        ...
        for (const series of list) {
            ...
            if (!Number.isFinite(tmdb) || tmdb <= 0) continue;
            const entry = { series, instance };
            index.set(tmdb, entry);
            if (!byTmdb.has(tmdb)) byTmdb.set(tmdb, entry);
        }
        seriesCatalogCache.set(key, { at: now(), list, byTmdb: index });
    }));
...
export const createLibraryAvailability = (config = {}, options = {}) => {
    const catalogTimeoutMs = Number(options.catalogTimeoutMs) > 0 ? Number(options.catalogTimeoutMs) : 12000;
    const fetchOpts = {
    ...
    const getTvListStatus = async (tmdbId) => {
        ...
        const byTmdb = await loadSeriesCatalogIndex(config, fetchOpts);
        const hit = byTmdb.get(id);
        if (!hit) {
            return {
                mediaType: 'tv',
    ...
    const enrichItems = async (items = []) => {
        ...
        // Warm catalogs once for the batch.
        const needsMovies = items.some((item) => resolveMediaType(item) === 'movie');
        const needsTv = items.some((item) => resolveMediaType(item) === 'tv');
        if (needsMovies) await loadMovieCatalogIndex(config, fetchOpts);
        if (needsTv) await loadSeriesCatalogIndex(config, fetchOpts);
