...
const assertValidArrRouting = async (config, mediaType, routing, fetchOpts) => {
    ...
    const options = await getPortalArrServiceOptions(config, mediaType, routing.serverId, {
        ...fetchOpts,
    ...
    if (Number.isFinite(languageProfileId) && (options.languageProfiles || []).length) {
        const allowed = new Set((options.languageProfiles || []).map((p) => Number(p.id)));
        if (!allowed.has(languageProfileId)) {
            const err = new Error('Invalid language profile for the selected *arr server.');
            err.status = 400;
            throw err;
        }
    }
...
export const createPortalRequestService = ({
    ...
    const enrichDto = async (record, { detail = false } = {}) => {
        let working = record;
        // Backfill missing TV posters from TheTVDB (persisted so the list stays fast).
        if (
            (working?.mediaType === 'tv' || working?.type === 'tv')
            && !working?.posterPath
            && Number(working?.tmdbId) > 0
            && String(config?.tvdbApiKey || '').trim()
        ) {
            try {
                const tvdbPoster = await resolveTvdbPosterForTmdbShow(config, working.tmdbId, {
                    fetchImpl,
                    tvdbIdHint: working?.meta?.tvdbId || working?.tvdbId || null,
                });
                if (tvdbPoster?.posterPath && working?.id != null) {
                    working = await store.update(working.id, {
                        posterPath: tvdbPoster.posterPath,
                        meta: {
                            ...(working.meta || {}),
                            posterSource: 'tvdb',
                            tvdbId: tvdbPoster.tvdbId,
                        },
                    }) || {
                        ...working,
                        posterPath: tvdbPoster.posterPath,
                ...
                // Keep placeholder if TVDB is unavailable.
            }
        }
