...
export const createPortalRequestService = ({
    ...
    const enrichDto = async (record, { detail = false } = {}) => {
        ...
        const dto = mapPortalRecordToDto(record, user || {});
        ...
        if (detail && dto.type === 'tv' && dto.tmdbId) {
            try {
                ...
                if (apiKey) {
                    ...
                    const wantsAll = record?.seasons === 'all'
                        || record?.seasons == null
    ...
    const createMemberRequest = async (user, body = {}) => {
        ...
        if (body.serverId != null || body.profileId != null || body.rootFolder || body.languageProfileId != null) {
    ...
    const getMemberRequestOptions = async (user, { mediaType, mediaId } = {}) => {
        ...
        // Request modal only needs season-level stats — avoid episode fan-out that can hang Sonarr.
        const yearRaw = String(details?.firstAirDate || details?.releaseDate || '').slice(0, 4);
        const year = Number(yearRaw);
        const statusPayload = type === 'tv'
            ? await library.getTvListStatus(tmdbId, {
                title: details?.name || details?.title || '',
                year: Number.isFinite(year) && year > 1900 ? year : null,
            }).catch(() => null)
            : await library.getMediaStatus(type, tmdbId).catch(() => null);
        ...
        const sessionKey = String(user?.id || user?.plexId || '').trim();
        const resolvedUser = sessionKey ? await resolveUser(sessionKey).catch(() => null) : null;
