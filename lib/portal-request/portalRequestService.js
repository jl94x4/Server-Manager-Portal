...
export const createPortalRequestService = ({
    ...
    const listAdminRequests = async ({ filter = 'pending', take = 20, skip = 0 } = {}) => {
        ...
        // Backfill missing TV posters for the visible page only (persist for next load).
        const recordById = new Map(records.map((row) => [String(row.id), row]));
        await Promise.all(page.map(async (dto, index) => {
            if (dto?.type !== 'tv' || dto?.posterUrl || dto?.posterPath) return;
            const record = recordById.get(String(dto.id));
            if (!record) return;
            const updated = await resolveMissingTvPoster(record, { timeoutMs: 2000 });
            if (updated?.posterPath && updated.posterPath !== record.posterPath) {
