...
export const createPortalRequestService = ({
    ...
    const enrichDto = async (record, { detail = false } = {}) => {
        let working = record;
        // Detail view only: try a short TVDB poster backfill so approve UI can show art.
        // Never block list endpoints on TheTVDB (that caused gateway 502s / UI flashes).
        if (
            detail
            ...
            try {
                const tvdbPoster = await Promise.race([
                    resolveTvdbPosterForTmdbShow(config, working.tmdbId, {
                        fetchImpl,
    ...
    const pushRecordToArr = async (record) => {
        ...
        try {
            ...
            return {
                ok: false,
                reason: error?.message || 'Invalid *arr routing for this request.',
            };
        }
