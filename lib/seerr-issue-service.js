...
export const createSeerrIssueService = ({
    ...
    const listIssuePage = async (config, { filter = 'all', take = 20, skip = 0, sort = 'added', createdBy = null } = {}) => {
        ...
        if (createdBy != null) params.set('createdBy', String(createdBy));
    ...
    const listMemberIssues = async (config, sessionUser, { filter = 'all', take = 20, skip = 0 } = {}) => {
        ...
        const payload = await listIssuePage(config, {
            filter: filter === 'open' || filter === 'resolved' ? filter : 'all',
            take,
            skip,
            createdBy: userId,
        });
        ...
        const filtered = filter === 'open'
            ? results.filter(isOpenSeerrIssue)
            : filter === 'resolved'
                ? results.filter((item) => !isOpenSeerrIssue(item))
                : results;

        return {
            userMapped: true,
            results: filtered.map((item) => mapSeerrIssueToDto(item, linkBaseUrl)),
            pageInfo: payload?.pageInfo || { pages: 1, results: filtered.length, page: 1 },
        };
    };
    ...
    const getMemberIssueCounts = async (config, sessionUser) => {
        ...
        const payload = await listIssuePage(config, { filter: 'all', take: 100, skip: 0, createdBy: userId });
        const results = Array.isArray(payload?.results) ? payload.results : [];
