...
const seriesTmdbIndexCache = new Map();
...
const loadCachedSeriesList = async (instance, fetchOpts) => {
    ...
    for (const entry of normalized) {
        const tmdb = Number(entry?.tmdbId);
        if (Number.isFinite(tmdb) && tmdb > 0) tmdbIndex.set(tmdb, entry);
    }
    seriesTmdbIndexCache.set(cacheKey, { at: Date.now(), index: tmdbIndex });
...
const findSeriesInInstance = async (instance, tmdbId, fetchOpts) => {
    const cacheKey = String(instance?.id || instance?.url || '');
    const indexed = seriesTmdbIndexCache.get(cacheKey);
    if (indexed && Date.now() - indexed.at < SERIES_LIST_CACHE_MS) {
        const hit = indexed.index.get(Number(tmdbId));
        if (hit?.id) return hit;
    }

    const list = await loadCachedSeriesList(instance, fetchOpts);
    return list.find((entry) => Number(entry?.tmdbId) === Number(tmdbId)) || null;
};
...
export const fetchSonarrLibraryStatusForShow = async (config, tmdbId, opts = {}) => {
    ...
    if (!resolved) {
        const miss = { matched: false };
        setCachedStatus(id, miss);
        return miss;
    }
