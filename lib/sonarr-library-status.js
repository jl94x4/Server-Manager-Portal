...
const findSeriesInInstance = async (instance, tmdbId, fetchOpts, { config = null, title = '', year = null } = {}) => {
    ...
    try {
        ...
        // Ignore lookup failures; caller treats as unmatched.
    }

    // Last resort: TMDB → TVDB then re-check (covers cold external_ids cache races).
    if (config) {
        const tvdbId = await resolveTvdbIdFromTmdb(config, tmdbId, { fetchImpl: fetchOpts?.fetchImpl || fetch });
        if (tvdbId && indexes.byTvdb.has(tvdbId)) {
