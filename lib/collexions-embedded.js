...
const candidates = () => {
    ...
    return [
        ...
        path.join(REPO_ROOT, 'ColleXions-WebUI-main'),
        '/app/collexions',
    ].filter(Boolean);
};
...
const startWorker = async ({ configDir, serviceKey, log = () => {} }) => {
...
export const syncCollexionsEmbeddedWorker = async (config, { configDir, log = () => {} } = {}) => {
    ...
    startingPromise = startWorker({ configDir, serviceKey, log })
        .then(() => ({ running: true, bundled: true }))
